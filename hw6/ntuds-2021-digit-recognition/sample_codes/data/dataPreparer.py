...
class Data:
    def __init__(self, args, data_path, label_path):
        ...
        inference_data_path = data_path.replace('train', 'test')
        ...
        # change here to load inference data !!!
        # change here to load inference data !!!
        # change here to load inference data !!!
        if label_path is not None:
            test_label_path = label_path.replace('train', 'valid')
        
        inference_dataset = DataPreparation(root=args,  
                                       data_path=inference_data_path,
                                       label_path=None,
                                       transform=transform)
        
        self.loader_inference = DataLoader(
            inference_dataset, batch_size=args.train_batch_size, shuffle=False, 
            num_workers=2
            )
